...
class IpReassembly : public Handler {
 ...
 public:
  explicit IpReassembly(std::unique_ptr<Handler> next_reassemble_)
      : Handler(std::move(next_reassemble_)), ip_reassembly_() {}
...
class HttpReassembly : public Handler {
 public:
  explicit HttpReassembly(std::unique_ptr<Handler> next_handler) : Handler(std::move(next_handler)), fragment_map_(), http_parser() {};
  void handle(std::unique_ptr<Data> data) override;
