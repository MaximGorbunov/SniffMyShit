...
darwin_images=("pcap-darwin-arm64" "pcap-darwin-amd64")
...
for i in "${!darwin_images[@]}"; do
  image=${darwin_images[$i]}
  ...
  if docker image inspect "$image" > /dev/null 2>&1; then
    ...
    docker build --platform linux/amd64 -t $image -f Dockerfile_darwin .
    if [ $? -eq 0 ]; then
...
images=("${linux_images[@]}" "${darwin_images[@]}")
build_dirs=("linux-armv5-musl" "linux-armv6-musl" "linux-armv7l-musl" "linux-arm64" "linux-amd64" "darwin-amd64" "darwin-arm64")
